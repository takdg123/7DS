...
function clipRectangle(x05, y05, x12, y12) {
  ...
  function corner(p, direction) {
    return abs2(p[0] - x05) < epsilon ? direction > 0 ? 0 : 3 : abs2(p[0] - x12) < epsilon ? direction > 0 ? 2 : 1 : abs2(p[1] - y05) < epsilon ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
  }
...
function graticule() {
  ...
  function lines() {
    return range(ceil(X02 / DX) * DX, X12, DX).map(X3).concat(range(ceil(Y02 / DY) * DY, Y12, DY).map(Y3)).concat(range(ceil(x05 / dx) * dx, x12, dx).filter(function(x2) {
      return abs2(x2 % DX) > epsilon;
    }).map(x)).concat(range(ceil(y05 / dy) * dy, y12, dy).filter(function(y2) {
      return abs2(y2 % DY) > epsilon;
    }).map(y));
  }
...
function resample(project, delta2) {
  function resampleLineTo(x05, y05, lambda0, a0, b0, c0, x12, y12, lambda1, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = sqrt(a * a + b * b + c * c), phi2 = asin(c /= m), lambda2 = abs2(abs2(c) - 1) < epsilon || abs2(lambda0 - lambda1) < epsilon ? (lambda0 + lambda1) / 2 : atan2(b, a), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x05, dy2 = y2 - y05, dz = dy * dx2 - dx * dy2;
...
function projectionMutator(projectAt) {
  ...
  projection2.clipAngle = function(_) {
    return arguments.length ? (preclip = +_ ? circle_default(theta = _ * radians) : (theta = null, antimeridian_default), reset()) : theta * degrees2;
  };
...
aitoffRaw.invert = function(x, y) {
  ...
  do {
    ...
    x12 -= dx, y12 -= dy;
  } while ((abs3(dx) > epsilon3 || abs3(dy) > epsilon3) && --i > 0);
  return [x12, y12];
};
...
if (!window.$RefreshReg$ || !window.$RefreshSig$ || !window.$RefreshRuntime$) {
  ...
  window.$RefreshReg$ = (type2, id2) => {
    window.$RefreshRuntime$.register(type2, '"app/routes/plot.tsx"' + id2);
  };
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1707976161978.6538";
}
var ScatterGeoPlot = ({
  data,
  width,
  ...
  _s();
  const svgRef = (0, import_react.useRef)();
  (0, import_react.useEffect)(() => {
    const svg = select_default2(svgRef.current);
    const projection2 = aitoff_default().scale(100).translate([width / 2, height / 2]);
    const graticule2 = graticule();
    svg.append("path").datum(graticule2).attr("class", "graticule").attr("d", path_default().projection(projection2)).attr("fill", "none").attr("stroke", "#ccc").attr("stroke-width", 0.5).attr("stroke-opacity", 0.5);
    const circles = svg.selectAll("circle").data(data.filter((d) => typeof d.ra === "number" && typeof d.dec === "number")).enter().append("circle").attr("cx", (d) => projection2([parseFloat(d.ra), parseFloat(d.dec)])[0]).attr("cy", (d) => projection2([parseFloat(d.ra), parseFloat(d.dec)])[1]).attr("r", 5).attr("fill", "blue").attr("stroke", "black").attr("stroke-width", 1);
    const tooltip = select_default2(svgRef.current.parentNode).append("div").style("position", "absolute").style("background-color", "white").style("border", "1px solid black").style("padding", "5px").style("visibility", "hidden");
  ...
  return /* @__PURE__ */ (0, import_jsx_dev_runtime.jsxDEV)("svg", { ref: svgRef, width, height }, void 0, false, {
    fileName: "app/routes/plot.tsx",
    lineNumber: 63,
    columnNumber: 10
  }, this);
};
_s(ScatterGeoPlot, "89Ty783ABEwsfMbSOeu9vscWF34=");
_c = ScatterGeoPlot;
var plot_default = ScatterGeoPlot;
var _c;
$RefreshReg$(_c, "ScatterGeoPlot");
...
//# sourceMappingURL=/build/_shared/chunk-QKLUKKVU.js.map
